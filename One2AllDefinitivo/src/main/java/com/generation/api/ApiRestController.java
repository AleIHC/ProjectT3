...
public class ApiRestController {
	...
	public ResponseEntity<Marcador> saveMarcador(@RequestBody Marcador marcador) {
    ...
    //localhost:9080/api/guardar/categorias
    @PostMapping(value = "/guardar/categorias")
    public ResponseEntity<Categoria> guardarCategorias(@RequestBody Categoria categoria){
        categoriaService.guardarCategoria(categoria);
        return new ResponseEntity<Categoria>(categoria, HttpStatus.OK);
    }
    
    @RequestMapping("/obtener/categorias")
    public List<Categoria> obtenerListaCategorias() {
        List<Categoria> listaCategorias = categoriaService.findAll();//obtener lista de marcadores
